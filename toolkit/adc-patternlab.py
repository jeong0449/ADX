...
SCRIPT_NAME="adc-patternlab.py"; VERSION="260802c"; VERSION_TEXT=f"{SCRIPT_NAME} {VERSION}"
...
def card(b,x,y,w=430,h=350,path=None):
    ...
    for subdiv,cells_per_beat in subdivision_cells.items():
        ...
        for (si,c),e in sorted(cells.items(),key=lambda item:(smap[item[0][0]],item[0][1])):
            ...
            p.append(f'<rect x="{xx+.6:.2f}" y="{yy+.6:.2f}" width="{max(.5,cell_w-1.2):.2f}" height="{max(.5,sh-1.2):.2f}" rx="1.2" class="slotcell accent{level}"><title>slot {si} {b.smap.slots[si].label}; raw {e.note}; velocity {e.vel}; duration {e.dur}; accent {level} ({accent}); subdivision {subdiv}</title></rect>')
        p.append('</g>')
    p.append('</g>')
